...
Results(iresu).Name2 = 'no control'; % Name to print on the graph legends
...
Results(iresu).Name = 'SC51'; % Simulation name
Results(iresu).Name2 = 'control'; % Name to print on the graph legends
...
filename = '';
set(gcf,'PaperUnits','Inches','PaperSize',[5 4],'PaperPosition',[0 0 5 4])
...
for inode = 1:NumMacroNodes
    for iroute = RoutesList
        if ismember(inode,Results(iresu).Route(iroute).NodePath)
            MacroNodesList = [MacroNodesList inode];
        end
    end
end
MacroNodesList = unique(MacroNodesList);
figure
opts.fontname = 'Arial';
opts.fontsize = 14;
opts.linewidth = 1;
...
opts.plotnumnodes = 0;
opts.exactsmooth = 1;
plotMacroNodes([],Results(iresu).Reservoir,1:NumRes,0,MacroNode,MacroNodesList,0,Results(iresu).Route,RoutesList,0,opts)
filename = ['_' Results(iresu).Name];
set(gcf,'PaperUnits','inches','PaperSize',[5 4],'PaperPosition',[0 0 5 4])
% print('-painters','-dpdf',['UserNetworks/' Results(iresu).Network '/img/' Results(iresu).Network '_macronodes' filename '.pdf'])
...
%% Total network traveled time TTT (delay)
%--------------------------------------------------------------------------
...
LW = 1*ones(1,Nresu); % line widths for results
clight = linspace(0,(Nresu-1)/Nresu,Nresu); % color brightness (0 to 1) for results
% cmap = arrayextension(cmap_perso2,Nresu,'row'); % colors for reservoirs
cmap = zeros(1,Nresu)'*[1 1 1]; % colors for results

% Figure and subfigure options
figwidth = 5; % whole figure width [inches]
figheight = 2; % whole figure height [inches]
...
for iresu = 1:Nresu
    Temp_t = Results(ResuList(iresu)).SimulTime;
    Temp_npeak = zeros(1,length(Temp_t));
    for r = ResList
        Temp_n = Results(ResuList(iresu)).Reservoir(r).Acc;
        %Temp_npeak = Temp_npeak + Temp_n - Temp_n(end);
        Temp_npeak = Temp_npeak + Temp_n;
    end
    Temp_delay = zeros(1,length(Temp_t));
    for j = 1:(length(Temp_t)-1)
        Temp_delay(j+1) = max([sum(Temp_npeak(1:j))*Simulation.TimeStep 0]); % TTT at t: integral of total acc from 0 to t
    end
    hp1(i) = plot(Temp_t,Temp_delay./3600,...
        'linestyle',sline{iresu},'color',lightencolor(cmap(iresu,:),clight(iresu)),'linewidth',LW(iresu));
    strleg{i} = Results(ResuList(iresu)).Name2;
    i = i + 1;
end
hold off
axis([TimeRange TTTRange])
xlabel('time \itt \rm[s]','FontName',fontname,'FontSize',FS)
ylabel('total traveled time \itTTT(t) \rm[h]','FontName',fontname,'FontSize',FS)
hleg = legend(hp1,strleg);
set(hleg,'Location','NorthWest','FontName',fontname,'FontSize',FS)
set(gca,'FontName',fontname,'FontSize',FS)
set(gcf,'Position',[10 10 1000 500])

% set(gcf,'PaperUnits','inches')
% set(gcf,'PaperSize',[figwidth figheight],'PaperPosition',[0 0 figwidth figheight])
% print('-painters','-dpdf',['UserNetworks/' Results(1).Network '/img/' Results(iresu).Network '_TTTtot_' filename '.pdf'])


%% Total traveled time TTT in reservoirs
%--------------------------------------------------------------------------
...
% cmap = [linspace(0.4,0.6,(Nresu-1)) 0]'*[1 1 1]; % colors for results
cmap = zeros(1,Nresu)'*[1 1 1]; % colors for results
...
for ifig = 1:Nfig
    ...
    for iresu = 1:Nresu
        ...
        Temp_delay = zeros(1,length(Temp_t));
        for j = 1:(length(Temp_t)-1)
            Temp_delay(j+1) = max([sum(Temp_n(1:j))*Simulation.TimeStep 0]); % TTT at t: integral of total acc from 0 to t
        end
        hp1(i) = plot(Temp_t,Temp_delay./3600,...
            'linestyle',sline{iresu},'color',lightencolor(cmap(iresu,:),clight(iresu)),'linewidth',LW(iresu));
        strleg{i} = Results(ResuList(iresu)).Name2;
    ...
    if mod(ifig,Ncol) == 1 || Ncol == 1
        ylabel('total traveled time \itTTT^r \rm[h]','FontName',fontname,'FontSize',FS)
    end
    if ifig == 1
        hleg = legend(hp1,strleg);
        legpos = get(hleg,'Position');
...
if PrintFig == 1
    set(gcf,'PaperUnits','inches')
    set(gcf,'PaperSize',[figwidth figheight],'PaperPosition',[0 0 figwidth figheight])
    print('-painters','-dpdf',['UserNetworks/' Results(1).Network '/img/' Results(iresu).Network '_TTT_' filename '.pdf'])
else
    set(gcf,'Position',[10 10 1000 700])
end
...
% Plot options
ResList = 1:NumRes; % list of reservoirs
MacroNodesList = [Simulation.Control(:).MacroNodesID]; % list of macro nodes
ResuList = [1 2]; % list of results
TimeRange = [0 Simulation.Duration]; % [s]
FlowRange = [0 1]; % [h]
PlotNodeCapacity = 1; % 1: plot the node capacity / 0: do not plot it
Nfig = length(MacroNodesList); % number of subfigures
Nresu = length(ResuList); % number of results
...
[colindex, rowindex] = ind2sub([Ncol Nrow],1:Nfig);
marginleft = 0.1; % figure relative left margin
marginbottom = 0.15; % figure relative bottom margin
intermarginright = 0.15; % subplot relative right margin
intermarginbottom = 0.05; % subplot relative bottom margin
intermargintop = 0.2; % subplot relative top margin
figwidth = 6.5; % whole figure width [inches]
...
for ifig = 1:Nfig
    ...
    for iresu = 1:Nresu
        Temp_t = Results(ResuList(iresu)).SimulTime;
        r = MacroNode(MacroNodesList(ifig)).ResID(1);
        i_n = find(Results(ResuList(iresu)).Reservoir(r).MacroNodesID == MacroNodesList(ifig));
        Temp_indexes = Results(ResuList(iresu)).Reservoir(r).NodeRoutesIndex{i_n};
        if strcmp(MacroNode(MacroNodesList(ifig)).Type,'origin') || ...
                strcmp(MacroNode(MacroNodesList(ifig)).Type,'externalentry')
            Temp_q = sum(Results(ResuList(iresu)).Reservoir(r).InflowPerRoute(Temp_indexes,:),1);
        else
            Temp_q = sum(Results(ResuList(iresu)).Reservoir(r).OutflowPerRoute(Temp_indexes,:),1);
        end
        if PlotNodeCapacity == 1
            Temp_t2 = MacroNode(MacroNodesList(ifig)).Capacity.Time;
            Temp_cap = MacroNode(MacroNodesList(ifig)).Capacity.Data;
            stairs(Temp_t2,Temp_cap,'-','color',lightencolor(cmap(iresu,:),0.8),'linewidth',3)
        end
        hp1(i) = plot(Temp_t,Temp_q,...
            'linestyle',sline{iresu},'color',lightencolor(cmap(iresu,:),clight(iresu)),'linewidth',LW(iresu));
        strleg{i} = Results(ResuList(iresu)).Name2;
        i = i + 1;
    end
    ...
    axis([TimeRange FlowRange])
    %xlim(TimeRange)
    title(['(' figindex(ifig) ') - Node ' int2str(MacroNodesList(ifig))],'FontName',fontname,'FontWeight','Bold','FontSize',FS1)
    %xticks(xt)
    %xticklabels(xtl)
    if ifig + Ncol > Nfig
        xlabel('time \itt \rm[s]','FontName',fontname,'FontSize',FS)
    ...
    if mod(ifig,Ncol) == 1 || Ncol == 1
        ylabel('flow through node \itq^m \rm[veh/s]','FontName',fontname,'FontSize',FS)
    end
    if ifig == 1
        hleg = legend(hp1,strleg);
        legpos = get(hleg,'Position');
        %set(hleg,'Position',[0.6 -0.02 legpos(3) legpos(4)],'FontName',fontname,'FontSize',0.8*FS)
        set(hleg,'Position',[0.6 0.01 legpos(3) legpos(4)],'FontName',fontname,'FontSize',FS) % for 2 col in fig
        %set(hleg,'Position',[0.7 0.05 legpos(3) legpos(4)],'FontName',fontname,'FontSize',0.8*FS) % for 3 col in fig
    end
    xj = marginleft + (colindex(ifig) - 1)*(1 - marginleft)/Ncol; % left pos
    yi = marginbottom + (Nrow - rowindex(ifig) + intermarginbottom)*(1 - marginbottom)/Nrow; % bottom pos
...
if PrintFig == 1
    set(gcf,'PaperUnits','inches')
    set(gcf,'PaperSize',[figwidth figheight],'PaperPosition',[0 0 figwidth figheight])
    print('-painters','-dpdf',['UserNetworks/' Results(1).Network '/img/' Results(iresu).Network '_nodeflow_' filename '.pdf'])
else
    set(gcf,'Position',[10 10 1000 700])
end
